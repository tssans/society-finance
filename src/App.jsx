...
const emptyTx = (type) => ({ type, account_id: ACCOUNTS[0].id, date: today(), amount: "", category: type==="income"?INCOME_CATS[0]:EXPENSE_CATS[0], description: "", ref: "" });
...
function KPI({ label, value, color, icon }) {
  return (
    <div style={S.kpi}>
      ...
      <div><div style={{ ...S.kpiVal, color }}>{value}</div><div style={S.kpiLabel}>{label}</div></div>
    </div>
  );
}
...
function Stat({ label, value, color }) {
  return (
    <div style={{ flex:1, textAlign:"center" }}>
      <div style={{ fontSize:13, fontWeight:800, color, letterSpacing:"-0.02em" }}>{value}</div>
      <div style={{ fontSize:10, color:"#9ca3af", marginTop:2 }}>{label}</div>
    </div>
  );
}

function TxRow({ e, acc, expanded, attachments, onExpand, onEdit, onDelete, onDeleteAtt }) {
  const inc = e.type === "income";
  return (
    <div style={{ borderBottom:"1px solid #f3f4f6" }}>
      <div style={S.row}>
        <div style={{ ...S.rowDot, background:inc?"#dcfce7":"#ffedd5", color:inc?"#15803d":"#c2410c" }}>{inc?"↑":"↓"}</div>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={S.rowTitle}>{e.category}{e.ref?` · ${e.ref}`:""}</div>
          <div style={S.rowSub}>
            {fmtDate(e.date)}
            {acc && <span style={{ ...S.accTag, background:acc.bg, color:acc.color }}>{acc.shortName}</span>}
            {e.description?` · ${e.description}`:""}
          </div>
        </div>
        <div style={{ fontWeight:700, fontSize:13, color:inc?"#15803d":"#c2410c", flexShrink:0 }}>{inc?"+":"-"}{fmt(+e.amount)}</div>
        <div style={{ display:"flex", gap:2, flexShrink:0 }}>
          <button style={S.iconBtn} onClick={onExpand}>📎</button>
          <button style={S.iconBtn} onClick={onEdit}>✎</button>
          <button style={{ ...S.iconBtn, color:"#f43f5e" }} onClick={onDelete}>✕</button>
        </div>
      </div>
      {expanded && (
        <div style={S.attPanel}>
          {!attachments && <span style={{ fontSize:12, color:"#9ca3af" }}>Loading…</span>}
          {attachments && attachments.length===0 && <span style={{ fontSize:12, color:"#9ca3af" }}>No attachments</span>}
          {attachments && attachments.map(att => (
            <div key={att.id} style={S.attRow}>
              <a href={att.file_url} target="_blank" rel="noreferrer" style={S.attLink}>📄 {att.file_name}</a>
              <button style={{ ...S.iconBtn, color:"#f43f5e", fontSize:12 }} onClick={() => onDeleteAtt(att)}>✕</button>
            </div>
          ))}
        </div>
      )}
...
function Empty({ text }) {
  return <div style={{ color:"#9ca3af", fontSize:13, textAlign:"center", padding:"28px 0" }}>{text}</div>;
}
...
export default function App() {
  ...
  const saveTx = async () => {
    ...
    try {
      let txId = form.id;
      if (txId) {
        await api(`transactions?id=eq.${txId}`, { method:"PATCH", body: JSON.stringify({ type:form.type, account_id:form.account_id, date:form.date, amount:+form.amount, category:form.category, description:form.description, ref:form.ref }) });
      } else {
        const [created] = await api("transactions", { method:"POST", body: JSON.stringify({ type:form.type, account_id:form.account_id, date:form.date, amount:+form.amount, category:form.category, description:form.description, ref:form.ref }) });
        txId = created.id;
      }
      for (const file of newFiles) {
        ...
        await storage(`object/${BUCKET}/${path}`, { method:"POST", body:fd, headers:{} });
        await api("attachments", { method:"POST", body: JSON.stringify({ transaction_id:txId, file_name:file.name, file_url:`${SUPABASE_URL}/storage/v1/object/public/${BUCKET}/${path}` }) });
      }
      setAttachments(prev => { const n={...prev}; delete n[txId]; return n; });
      setNewFiles([]); setForm(null); setTab("ledger"); setActiveNav("ledger");
      showToast("Entry saved ✓"); await loadAll();
    } catch(e) { showToast("Save failed: " + e.message, false); }
    setSaving(false);
  };

  const deleteTx = async (id) => {
    try { await api(`transactions?id=eq.${id}`, { method:"DELETE" }); setTxs(prev => prev.filter(t => t.id!==id)); showToast("Entry deleted"); }
  ...
  const submitFeedback = async () => {
    ...
    try {
      ...
      showToast("Feedback submitted ✓"); setFeedback(emptyFeedback());
  ...
  return (
    <div style={S.root} onClick={() => tooltip && setTooltip(null)}>
      <header style={S.header}>
        <div style={S.headerInner}>
          <div style={S.brand}>
            ...
            <div><div style={S.brandName}>Society Finance</div><div style={S.brandSub}>Live · Supabase</div></div>
          </div>
      ...
      <main style={S.main}>
                ...
                <div key={acc.id} style={{ ...S.accCard, borderLeft:`4px solid ${acc.color}` }}>
                  <div style={S.accCardTop}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      ...
                      <div><div style={S.accName}>{acc.name}</div><span style={{ ...S.badge, background:acc.bg, color:acc.color }}>{acc.shortName}</span></div>
                    </div>
            ...
            <div style={S.card}>
              ...
              {txs.slice(0,6).map(e => (
                <TxRow key={e.id} e={e} acc={ACC_MAP[e.account_id]}
                  expanded={expandedTx===e.id} attachments={attachments[e.id]}
                  ...
                  onDelete={() => { if(confirm("Delete?")) deleteTx(e.id); }}
              ...
              return (
                <div key={acc.id} style={{ ...S.accDetailCard, borderTop:`3px solid ${acc.color}` }}>
                  <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}>
                    ...
                    <div><div style={S.accDetailName}>{acc.name}</div><span style={{ ...S.badge, background:acc.bg, color:acc.color }}>{acc.shortName}</span></div>
                  </div>
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:12 }}>
                    ...
                    <button style={{ ...S.viewBtn, color:acc.color, borderColor:acc.color }} onClick={() => { setFilter(f=>({...f,accountId:acc.id})); navTo("ledger"); }}>View Ledger →</button>
                  </div>
                </div>
              );
            })}
        ...
        {tab==="ledger" && (
          <div>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
              ...
              <span style={{ fontSize:13 }}>Net: <strong style={{ color:filtered.reduce((s,e)=>e.type==="income"?s+ +e.amount:s- +e.amount,0)>=0?"#22c55e":"#f43f5e" }}>{fmt(filtered.reduce((s,e)=>e.type==="income"?s+ +e.amount:s- +e.amount,0))}</strong></span>
            </div>
            <div style={S.card}>
              {filtered.length===0 && <Empty text="No records match filters." />}
              {filtered.map(e => (
                <TxRow key={e.id} e={e} acc={ACC_MAP[e.account_id]}
                  expanded={expandedTx===e.id} attachments={attachments[e.id]}
                  onExpand={() => { setExpandedTx(expandedTx===e.id?null:e.id); loadAttachments(e.id); }}
          ...
          <div style={S.formCard}>
            ...
            <Fld label="Account">
              <div style={{ display:"flex", gap:8, marginBottom:8 }}>
                {ACCOUNTS.map(acc => (
                  <button key={acc.id} style={{ ...S.accChip, background:form.account_id===acc.id?acc.color:acc.bg, color:form.account_id===acc.id?"#fff":acc.color, border:`1.5px solid ${acc.color}` }} onClick={()=>setForm(f=>({...f,account_id:acc.id}))}>
                    {acc.icon} {acc.shortName}
                  </button>
                ))}
              </div>
              {form.account_id && <div style={S.chipDesc}>{ACC_MAP[form.account_id]?.description}</div>}
            </Fld>
            <Fld label="Date"><input type="date" style={S.input} value={form.date} onChange={e=>setForm(f=>({...f,date:e.target.value}))}/></Fld>
            <Fld label="Amount (₹)"><input type="number" style={S.input} placeholder="0" value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))}/></Fld>
            <Fld label="Category">
              <select style={S.input} value={form.category} onChange={e=>setForm(f=>({...f,category:e.target.value}))}>
                {(form.type==="income"?INCOME_CATS:EXPENSE_CATS).map(c=><option key={c}>{c}</option>)}
              </select>
            </Fld>
            <Fld label="Reference / Flat No."><input type="text" style={S.input} placeholder="e.g. A-101" value={form.ref} onChange={e=>setForm(f=>({...f,ref:e.target.value}))}/></Fld>
            <Fld label="Description"><input type="text" style={S.input} placeholder="Brief note…" value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))}/></Fld>
            <Fld label="Attachments (invoices, photos, receipts)">
              <label style={S.fileLabel}>
                <input type="file" multiple accept="image/*,.pdf" style={{ display:"none" }} onChange={e=>setNewFiles(prev=>[...prev,...Array.from(e.target.files)])}/>
                📎 Tap to attach files
              </label>
              {newFiles.map((f,i) => (
                <div key={i} style={S.fileChip}>
                  <span style={{ flex:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{f.name}</span>
                  <button style={S.fileChipDel} onClick={()=>setNewFiles(prev=>prev.filter((_,j)=>j!==i))}>✕</button>
                </div>
              ))}
            </Fld>
            <div style={{ display:"flex", gap:10, justifyContent:"flex-end", marginTop:20 }}>
              <button style={S.btnCancel} onClick={()=>{ setTab("ledger"); setActiveNav("ledger"); }}>Cancel</button>
              <button style={{ ...S.btnSave, background:form.type==="income"?"#22c55e":"#f97316", opacity:saving?0.6:1 }} onClick={saveTx} disabled={saving}>{saving?"Saving…":"Save Entry"}</button>
            </div>
          </div>
        )}
        ...
        {tab==="feedback" && (
          <div>
            ...
            <div style={S.formCard}>
              <div style={{ fontSize:13, color:"#6b7280", marginBottom:16, lineHeight:1.5 }}>Submit bugs, feature requests, or suggestions.</div>
              <Fld label="Your Name"><input type="text" style={S.input} placeholder="e.g. Rajesh (A-204)" value={feedback.submitted_by} onChange={e=>setFeedback(f=>({...f,submitted_by:e.target.value}))}/></Fld>
              <Fld label="Type">
                <select style={S.input} value={feedback.type} onChange={e=>setFeedback(f=>({...f,type:e.target.value}))}>
                  {FEEDBACK_TYPES.map(t=><option key={t}>{t}</option>)}
                </select>
              </Fld>
              <Fld label="Message"><textarea style={{ ...S.input, minHeight:90, resize:"vertical" }} placeholder="Describe your feedback…" value={feedback.message} onChange={e=>setFeedback(f=>({...f,message:e.target.value}))}/></Fld>
              <button style={{ ...S.btnSave, background:"#6366f1", width:"100%", opacity:fbSaving?0.6:1 }} onClick={submitFeedback} disabled={fbSaving}>{fbSaving?"Submitting…":"Submit Feedback"}</button>
            </div>
            {feedbackList.length>0 && (
              <div style={{ marginTop:20 }}>
                <div style={S.groupLabel}>All Feedback ({feedbackList.length})</div>
                {feedbackList.map(fb => (
                  <div key={fb.id} style={S.fbCard}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6 }}>
                      <div><span style={S.fbType}>{fb.type}</span>{fb.submitted_by&&<span style={S.fbWho}> · {fb.submitted_by}</span>}</div>
                      <span style={{ ...S.fbStatus, background:fb.status==="open"?"#fef9c3":"#dcfce7", color:fb.status==="open"?"#a16207":"#15803d" }}>{fb.status}</span>
                    </div>
                    <div style={{ fontSize:13, color:"#374151", lineHeight:1.5 }}>{fb.message}</div>
                    <div style={{ fontSize:11, color:"#9ca3af", marginTop:6 }}>{fmtDate(fb.created_at?.split("T")[0])}</div>
                  </div>
                ))}
              </div>
            )}
      ...
      {toast && <div style={{ ...S.toast, background:toast.ok?"#22c55e":"#f43f5e" }}>{toast.msg}</div>}
    </div>
  );
}
